...
# 配置
MUSK_ORCHESTRATOR_DIR = Path("/Users/linweihao/project/MuskOrchestrator")
WORKSPACE_DIR = MUSK_ORCHESTRATOR_DIR
...
def archive_agent(agent_name: str, dry_run: bool = False) -> Dict:
    """归档单个Agent的输出"""
    source_dir = MUSK_ORCHESTRATOR_DIR / f"subagents/{agent_name}/workspace"
    target_dir = MUSK_ORCHESTRATOR_DIR / f"memory/agents/{agent_name}"
...
def extract_insights_from_learning(agent_name: str) -> Dict:
    ...
    learning_file = MUSK_ORCHESTRATOR_DIR / f"memory/agents/{agent_name}" / "LEARNING.md"
...
def merge_to_memory(agent_name: str, insights: List[str]) -> bool:
    """将洞察合并到主MEMORY.md"""
    memory_file = MUSK_ORCHESTRATOR_DIR / "MEMORY.md"
...
def cleanup_workspace(agent_name: str, dry_run: bool = True) -> Dict:
    ...
    3. 删除7天以上的旧报告
    """
    workspace_dir = MUSK_ORCHESTRATOR_DIR / f"subagents/{agent_name}/workspace"
...
def main():
    ...
    if cleanup:
        ...
        if agent_names:
            ...
            agents = [
                d.name
                for d in (MUSK_ORCHESTRATOR_DIR / "subagents").iterdir()
                if d.is_dir()
            ]
    ...
    # 保存归档报告
    if not dry_run:
        report_file = MUSK_ORCHESTRATOR_DIR / "archive-reports" / f"archive_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
